...
**关联文件**：
- `~/.hermes/config.yaml`
- `~/Code/hermes_anyrouter_proxy/proxy.py`
- `~/Code/hermes_anyrouter_proxy/template.json`
...
[ ] 3. 抽出模板（去业务字段，留校验字段）
       ...
       存到 ~/Code/hermes_anyrouter_proxy/template.json
       
[ ] 4. 部署生产代理
       ~/Code/hermes_anyrouter_proxy/proxy.py（见 §4.2）
...
**文件**：`~/Code/hermes_anyrouter_proxy/proxy.py` (~230 行)
...
def _patch_body(raw):
    ...
    template = json.load(open("~/Code/hermes_anyrouter_proxy/template.json"))
...
**文件**：`~/Code/hermes_anyrouter_proxy/template.json` (~1.7 KB)
...
| `HERMES_PROXY_TEMPLATE` | `~/Code/hermes_anyrouter_proxy/template.json` | 模板文件路径 |
...
<array>
    ...
    <string>-u</string>
    <string>/Users/hainingyu/Code/hermes_anyrouter_proxy/proxy.py</string>
</array>
...
<string>/Users/hainingyu/Code/hermes_anyrouter_proxy/proxy.log</string>
...
# 查看实时日志
tail -f ~/Code/hermes_anyrouter_proxy/proxy.log
...
**检查**：
```bash
ls -la ~/Code/hermes_anyrouter_proxy/template.json
python3 -c "import json; print(json.load(open('$HOME/Code/hermes_anyrouter_proxy/template.json')).keys())"
```
...
**检查日志**：
```bash
tail -50 ~/Code/hermes_anyrouter_proxy/proxy.log
```
...
**检查**：
```bash
python3 -c "import json; t=json.load(open('$HOME/Code/hermes_anyrouter_proxy/template.json')); print('stream' in t)"
```
应输出 `False`。
...
python3 <<'PY'
import json
p = '/Users/hainingyu/Code/hermes_anyrouter_proxy/template.json'
t = json.load(open(p))
t.pop('stream', None)
...
json.dump(template, open('/Users/hainingyu/Code/hermes_anyrouter_proxy/template.json', 'w'), indent=2, ensure_ascii=False)
PY

# 5. 同步代理脚本里的 beta 列表（如果有变化）
# 编辑 ~/Code/hermes_anyrouter_proxy/proxy.py 的 INJECTED_BETAS
...
# 2. 删除代理文件
rm ~/Code/hermes_anyrouter_proxy/proxy.py
rm ~/Code/hermes_anyrouter_proxy/template.json
rm ~/Code/hermes_anyrouter_proxy/proxy.log
...
如需透传更多字段，编辑 `~/Code/hermes_anyrouter_proxy/proxy.py` 的 `HERMES_OVERRIDE_KEYS`。
...
| 时间 | 事件 |
|---|---|
| T22 | 文件从 `~/.hermes/` 迁移到 `~/Code/hermes_anyrouter_proxy/` 独立项目 |
